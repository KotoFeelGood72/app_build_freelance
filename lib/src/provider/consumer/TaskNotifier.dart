...
class TaskNotifier extends StateNotifier<TaskState> {
  ...
  void updateTaskPrice(int taskPrice) { // taskPrice как int
...
Future<void> submitTask() async {
  ...
  state = state.copyWith(isLoading: true, errorMessage: null, isErrorShown: false);
...
Future<void> fetchTaskResponses(String taskId) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception("Ошибка при получении откликов: ${response.statusCode}");
    }
  } catch (e) {
    state = state.copyWith(
      isLoading: false,
