...
import 'package:app_build_freelance/src/utils/date_picker_utils.dart';
...
class _PickDateState extends State<PickDate> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        DateTime? pickedDate = await pickDate(context, initialDate: _selectedDate);
        if (pickedDate != null) {
          setState(() {
              ...
              children: [
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 16,
                      color: _selectedDate != null ? Colors.black : AppColors.gray,
                    ),
                  ),
                ),
                const Text(
