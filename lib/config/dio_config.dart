...
import 'token_storage.dart'; // Хранилище для токенов
...
onResponse: (response, handler) async {
  if (response.statusCode == 401) {
    ...
    final isRefreshed = await _handleTokenRefresh(response.requestOptions);

    if (isRefreshed) {
      try {
        // Если токен обновлен, повторяем запрос
        final retryResponse = await _retryRequest(response.requestOptions);
        return handler.resolve(retryResponse); // Успешно завершаем обработку
      } catch (retryError) {
  ...
  handler.next(response);
        ...
        onError: (DioError e, handler) async {
          print('Ошибка запроса: ${e.type}, Код ответа: ${e.response?.statusCode}');
          String errorMessage;

          // Обработка различных типов ошибок
          if (e.type == DioErrorType.connectionTimeout) {
            errorMessage = 'Время ожидания соединения истекло. Проверьте подключение.';
            ...
            switch (e.response?.statusCode) {
              ...
              case 401:
                ...
                if (isRefreshed) {
                  ...
                  _redirectToLogin();
                  errorMessage = 'Не удалось обновить токен. Перейдите на страницу входа.';
                }
                break;
              case 403:
